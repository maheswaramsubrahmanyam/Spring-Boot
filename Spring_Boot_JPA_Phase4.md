# Goodreads API – Phase 4 (Spring Boot JPA – Many-to-Many)
...
- Book ↔ Author relationship using `@ManyToMany`
- `book_author` join table
- API to retrieve all authors of a book
...
The current project contains two different JPA relationships.
...
| GET | `/books/{bookId}/authors` | Get all authors of a book |
...
This creates a foreign-key relationship from the `book` table to the `publisher` table.
...
Book 2
   |
   +-- Author 2: Larry Niven
   |
   +-- Author 3: Jerry Pournelle
```

The `book_author` table stores this relationship:
...
Similarly, an author can be associated with multiple books.
...
These annotations are used with the bidirectional relationship to avoid repeatedly serializing the relationship in the JSON response.
...
So the JSON serialization is controlled using:
...
The service first finds the book and then returns its authors:
...
# Example – Get Authors of Book 2

Request:

```http
GET http://localhost:8080/books/2/authors
```

Based on the supplied `data.sql`, Book 2 has:
...
# Example – Get Authors of Book 3

Request:

```http
GET http://localhost:8080/books/3/authors
```
...
The relevant relationship rows are:
...
The book request uses the publisher relationship:
...
The current `BookJpaService` validates the publisher ID by looking it up using:

```java
publisherJpaRepository.findById(publisherId).get();
```
...
The service gets the book and returns:
...
The application uses the file-based H2 database:
...
The supplied `data.sql` creates these authors:
...
6. Half Girlfriend
7. The Secret
8. The 3 Mistakes of My Life
```

---

# Initial Book–Author Relationships

The supplied `data.sql` contains:
...
```text
http://localhost:8080/books/1/publisher
```
...
The previous project contained:
...
Book had a relationship with Publisher:
...
The current implementation adds the Many-to-Many relationship:
...
This Goodreads API implementation contains three entities:
...
The project now contains two relationships:
...
The Many-to-Many relationship is maintained through:
...
The new API for the Many-to-Many relationship is:

```text
GET /books/{bookId}/authors
```

which retrieves the authors associated with a particular book.
